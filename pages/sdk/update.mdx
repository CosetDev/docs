...
Use [`Coset.update()`](docs/pages/sdk/reference) to perform a paid update.
...
On success, `update()` resolves to an [`IUpdate`](docs/pages/sdk/reference):
...
On success, the SDK also increments [`coset.spent`](docs/pages/sdk/reference) by `spent.total`.
...
Or use [`Coset.optionalUpdate()`](docs/pages/sdk/reference), which performs that check and triggers an update when needed, but always resolves with `null`:
...
Unlike `read()`, [`Coset.update()`](docs/pages/sdk/reference) **rejects** on failures.
